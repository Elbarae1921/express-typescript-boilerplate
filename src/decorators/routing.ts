...
import { Route, RequestMethod } from "../types/route.interface";
...
export const Controller = (path: string = '') => <T extends { new (...args: any[]): {} }>(constructor: T) => {
    path = fixPath(path);
...
export const Handler = (method: RequestMethod, path: string, ...middleware: RequestHandler[]) => {
    path = fixPath(path);
